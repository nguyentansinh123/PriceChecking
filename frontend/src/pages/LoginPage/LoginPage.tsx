...
import useNLogin from '@/hooks/AuthHooks/useNLogin'
...
const LoginPage = () => {
  ...
  const {isPending, error, NLoginMutation} = useNLogin()

  const handleLogin = (e: React.FormEvent<HTMLFormElement>)=>{
    e.preventDefault()
    NLoginMutation(loginData)
  }
    ...
    <AuthLayout isLogin>
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleLogin}>
          <InputField type="text" placeholder="Username" icon={<FaRegEnvelope />} value={loginData.email} onChange={(e)=> setLoginData({...loginData, email: e.target.value} )} />
          <InputField type="password" placeholder="Password" icon={<FaLock />} value={loginData.password} onChange={(e)=> setLoginData({...loginData, password: e.target.value} )}/>
